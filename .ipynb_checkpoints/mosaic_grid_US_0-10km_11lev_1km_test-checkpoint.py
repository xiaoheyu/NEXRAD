...
#     outPut_file = os.path.join(myDate,myTime,"nohup.txt")
    ...
    all_sites=get_allsites()
    start_dt = datetime.date(2018,1,1)  # custommized your starting date and end date here
    end_dt = datetime.date(2018,1,31)
    ...
    for dt in daterange(start_dt, end_dt):
        for myTime in TimeRange:
            ...
            if Path(os.path.join(dt.strftime("%Y-%m"),dt.strftime("%Y-%m-%d"), myTime,dt.strftime("%Y-%m-%d") + "_" + myTime + ".nc")).exists():
                
                print("%s exists, plot......" %(os.path.join(dt.strftime("%Y-%m"),dt.strftime("%Y-%m-%d"), myTime,dt.strftime("%Y-%m-%d") + "_" + myTime + ".nc")))
                try:
                    grid = pyart.io.read_grid(os.path.join(dt.strftime("%Y-%m"),dt.strftime("%Y-%m-%d"),
                                                           myTime,dt.strftime("%Y-%m-%d") + "_" + myTime + ".nc"))
